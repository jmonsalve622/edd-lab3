// Tabla hash con hashing cerrado y resolucion de colisiones por sondeo
// cuadratico (quadratic probing). Ante una colision, el salto crece de forma
// cuadratica: H(k, i) = (h(k) + i^2) mod N, lo que reduce el agrupamiento
// primario (primary clustering) respecto del sondeo lineal.
//
...
// Tabla hash de sondeo cuadratico con user_id (entero) como clave.
...
public:
    ...
    // La secuencia de sondeo prueba las posiciones h(k), h(k)+1, h(k)+4,
    // h(k)+9, ... (saltos i^2) hasta hallar la clave o una casilla libre.
    void insert(uint64_t clave) {
        ...
        while (tabla[(indice + i * i) % capacidad].estado == OCUPADO) {
            ...
            i++;
            // A diferencia del sondeo lineal, el cuadratico puede no visitar
            // todas las casillas, por lo que se acota la cantidad de intentos.
            if (i >= capacidad) {
...
// Tabla hash de sondeo cuadratico con user_screen_name (string) como clave.
// Identica en estructura a QuadraticProbingID, salvo por la funcion hash.
...
public:
    ...
    // Misma logica de insercion que en la version por id: sondeo cuadratico
    // hasta hallar la clave existente o una casilla libre.
...
int main(int argc, char* argv[]) {
    ...
    std::cout << readCount << ";quadratic_probing;user_id;" << numExperiments << ";"
              << meanTimeIds << ";" << stdDeviationIds << ";" << idTable.inMemorySize() / 1024 << "\n";

    std::cout << readCount << ";quadratic_probing;user_screen_name;" << numExperiments << ";"
